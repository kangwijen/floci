...
Against the hardened Compose image with IAM enforcement, replace `test`/`test` with operator root or participant IAM keys. Unsigned or dummy credentials return `403`. Use `is_ctf_credentials()` from `lib/ctf-env.sh` in shell helpers when you need to branch on enforcement mode.
